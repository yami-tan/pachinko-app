...
export default function PachinkoCalculatorComplete() {
  ...
  function openFirstHitDialog() {
    ...
    // ★ 貯玉スナップショット: 初期値から投資行で消費された分を差し引く
    //   例) storedBallsInput=2500, 投資行1000玉 → 大当たり前の貯玉=1500玉
    const _totalBallInvested=formMetrics.allBallInvestBalls||0;
    const _storedSnapshot=String(Math.max(0,numberOrZero(form.storedBallsInput)-_totalBallInvested));
    setFirstHitForm({
      label:`初当たり${nc}回目`,
      rounds:'0',
  ...
  function completeFirstHit(restartAfter=false) {
    ...
    applyFormUpdate(prev=>{
      ...
      return {
        ...
        // ★ 収支バグ修正: storedBallsInput は endBalls の有無にかかわらず常に更新する
        //   endBalls=0（未入力）でも貯玉消費は発生しているため storedAfter を反映必須
        ...
        ...(endBallsForRestart>0?{
          ...
          currentInputMode:'balls',
          // ★ 収支バグ修正: inheritedBalls は初期引き継ぎ値のみ（jackpot後に更新しない）
          //   更新すると useInheritedBal = -endBalls×rate になり currentBallsYen と相殺される
        }:{}),
      };
    });
                        ...
                        <div style={{ display:'flex', alignItems:'center', gap:8, padding:'10px 12px' }}>
                          ...
                          <div style={{ flex:1 }}>
                            ...
                            {entry.kind==='restart' ? (
                              ...
                              <div style={{ display:'flex', flexDirection:'column', gap:4 }}>
                                {/* 上段：種別切替（セグメントコントロール） + 前行コピー */}
                                <div style={{ display:'flex', alignItems:'center', gap:4 }}>
                                  <div style={{ flex:1, display:'flex', borderRadius:10, overflow:'hidden', border:`1.5px solid ${C.border}`, background:isDark?'rgba(255,255,255,0.04)':'#f1f5f9' }}>
                                    <button
                                      onClick={()=>entry.kind!=='balls'&&updateRateEntry(entry.id,'kind','balls')}
                                      style={{ flex:1, padding:'7px 2px', border:'none',
                                        cursor:entry.kind!=='balls'?'pointer':'default',
                                        background:entry.kind==='balls'?C.positive:'transparent',
                                        color:entry.kind==='balls'?'#fff':C.textMuted,
                                        fontSize:12, fontWeight:800, transition:'background 0.15s' }}
                                    >🎱 持ち玉</button>
                                    <button
                                      onClick={()=>entry.kind!=='cash'&&updateRateEntry(entry.id,'kind','cash')}
                                      style={{ flex:1, padding:'7px 2px', border:'none',
                                        cursor:entry.kind!=='cash'?'pointer':'default',
                                        background:entry.kind==='cash'?C.accent:'transparent',
                                        color:entry.kind==='cash'?'#fff':C.textMuted,
                                        fontSize:12, fontWeight:800, transition:'background 0.15s' }}
                                    >💴 現金</button>
                                  </div>
                                  {(()=>{
                  ...
                  <DialogContent className="max-w-sm rounded-3xl" onOpenAutoFocus={e=>e.preventDefault()}>
                    ...
                    <div className="space-y-2 max-h-[70vh] overflow-y-auto pr-1">
                      ...
                      <div className="rounded-xl border p-2" style={{ display:'flex', flexDirection:'column', gap:8 }}>
                        ...
                        <div style={{ background:isDark?'rgba(99,102,241,0.10)':'#f5f3ff', border:`2px solid ${C.primaryMid}`, borderRadius:16, padding:'14px 16px', display:'flex', flexDirection:'column', gap:10 }}>
                          {/* タイトル行 */}
                          <div style={{ display:'flex', alignItems:'center', gap:8 }}>
                            <span style={{ fontSize:20 }}>🏦</span>
                            <div style={{ flex:1 }}>
                              <div style={{ fontSize:13, fontWeight:800, color:C.primary, letterSpacing:'0.01em' }}>大当たり後の残り貯玉</div>
                              <div style={{ fontSize:11, color:C.textMuted, marginTop:2, lineHeight:1.4 }}>
                                {numberOrZero(firstHitForm.storedBallsSnapshot)>0
                                  ...
                                  :'大当たり後に残った貯玉を入力（なければ0のまま）'}
                              </div>
                            </div>
                            <div style={{ background:isDark?'rgba(99,102,241,0.15)':C.primaryLight, border:`1px solid ${C.primaryMid}`, borderRadius:8, padding:'3px 10px', flexShrink:0 }}>
                              <div style={{ fontSize:9, color:C.textMuted, textAlign:'center' }}>デフォルト</div>
                              <div style={{ fontSize:13, fontWeight:800, color:C.primary, textAlign:'center' }}>0玉</div>
                            </div>
                          </div>
                          {/* 貯玉スナップショット表示（あれば） */}
                          {numberOrZero(firstHitForm.storedBallsSnapshot)>0&&(
                            <div style={{ display:'flex', alignItems:'center', gap:6, background:isDark?'rgba(255,255,255,0.04)':'rgba(255,255,255,0.7)', borderRadius:10, padding:'8px 12px' }}>
                              <div style={{ flex:1, display:'flex', alignItems:'center', gap:4 }}>
                                <span style={{ fontSize:11, color:C.textMuted }}>大当たり前</span>
                                <span style={{ fontSize:14, fontWeight:700, color:C.textSecondary }}>{numberOrZero(firstHitForm.storedBallsSnapshot).toLocaleString()}玉</span>
                              </div>
                              <span style={{ fontSize:16, color:C.textMuted }}>→</span>
                              <div style={{ flex:1, display:'flex', alignItems:'center', gap:4, justifyContent:'flex-end' }}>
                                <span style={{ fontSize:11, color:C.textMuted }}>残り</span>
                                <span style={{ fontSize:14, fontWeight:800, color:numberOrZero(firstHitForm.storedAfter)>0?C.primary:C.textMuted }}>
                                  {numberOrZero(firstHitForm.storedAfter).toLocaleString()}玉
                                </span>
                              </div>
                            </div>
                          )}
                            ...
                            <div style={{ flex:1, position:'relative' }}>
                              <input
                                ...
                                style={{
                                  ...
                                  borderRadius:12, padding:'10px 48px 10px 12px',
                                  background:isDark?'rgba(255,255,255,0.06)':C.card,
                                  color:C.textPrimary, outline:'none',
                                }}
                              />
                              <span style={{
                                position:'absolute', right:12, top:'50%', transform:'translateY(-50%)',
                                fontSize:13, fontWeight:700, color:C.textMuted, pointerEvents:'none'
                              }}>玉</span>
                            </div>
                            {numberOrZero(firstHitForm.storedAfter)>0&&(
                              <button
                                onClick={()=>setFirstHitForm(p=>({...p,storedAfter:''}))}
                                style={{
                                  flexShrink:0, minWidth:64, borderRadius:12,
                                  border:`2px solid ${C.negativeBorder}`,
                                  background:C.negativeBg, color:C.negative,
                      ...
                      <div style={{ border:`2px solid ${C.amberBorder}`, borderRadius:16, overflow:'hidden', background:C.amberBg }}>
                        ...
                        <div style={{ padding:'10px 14px' }}>
                          ...
                          {(()=>{
                            ...
                            // ★ archived を除外して現区間のみの最終reading を取得
                            const lastReading=(form.rateEntries||[]).filter(e=>!e.archived).reduce((last,e)=>{
                            ...
                            // ─── 算出回転率の共通計算 ───
                            // ★ 修正: startRotation からの差分で正確に計算
                            //   リセット機: startRotation=0 → hitSpins がそのままセクション回転数
                            //   非リセット機: hitSpins - startRotation = このセクションの回転数
                            const startRot=numberOrZero(form.startRotation);
                            const sectionSpins=Math.max(0,hitSpins+remainingHolds-startRot);
                            ...
                            if(isCash){
                              ...
                              const upperBallsDeductYen=upperBalls*4;
                              const netAddCash=Math.max(0,cashInvest-upperBallsDeductYen);
                              // ★ firstHitForm.confirmedCash を使用（formMetrics との差異を防ぐ）
                              const confirmedSecCash=numberOrZero(firstHitForm.confirmedCash);
                              ...
                              // ★ firstHitForm.confirmedCash を使用
                              const confirmedCashFromTab=numberOrZero(firstHitForm.confirmedCash);
                              // ── 未記録分の計算（completeFirstHit と同一ロジック）──
                              const _confirmedFromDialog=numberOrZero(firstHitForm.confirmedBalls);
                              // snapshotBalls 方式の場合 unrecordedBalls は空 → 自動計算で補完
                              const _snapBalls=numberOrZero(firstHitForm.snapshotBalls);
                              const _remBalls=firstHitForm.remainingBalls!==''?numberOrZero(firstHitForm.remainingBalls):null;
                              const _unrecFromSnapshot=_snapBalls>0&&_remBalls!==null?Math.max(0,_snapBalls-_remBalls-upperBalls):0;
                              const _unrecBalls=firstHitForm.unrecordedBalls!==''
                                ?numberOrZero(firstHitForm.unrecordedBalls)
                                :_unrecFromSnapshot; // snapshotBalls 方式を優先
                              // ── 貯玉からの追加消費（storedBallsSnapshot - storedAfter）──
                              // 持ち玉がない場合（_snapBalls=0）: 貯玉がそのまま投資源
                              // ラスト投資行から大当たりまでに消費した貯玉分を未記録投資として加算
                              const _storedSnapForRate=numberOrZero(firstHitForm.storedBallsSnapshot);
                              const _storedAfterForRate=firstHitForm.storedAfter!==''?numberOrZero(firstHitForm.storedAfter):_storedSnapForRate;
                              const _additionalStoredConsumed=(_storedSnapForRate>0&&_snapBalls===0)
                                ?Math.max(0,_storedSnapForRate-_storedAfterForRate)
                                :0;
                              // ── 合計投資 ──
                              const totalBallsForPreview=_confirmedFromDialog+_unrecBalls+_additionalStoredConsumed;
                                ...
                                <div style={{ marginTop:10, display:'flex', flexDirection:'column', gap:6 }}>
                                  <div style={{ background:isDark?'rgba(245,158,11,0.1)':'white', borderRadius:10, padding:'8px 12px', border:`1px solid ${C.amberBorder}` }}>
                                    ...
                                    <div style={{ fontSize:12, color:C.amber, lineHeight:'1.8' }}>
                                      ...
                                      {_additionalStoredConsumed>0&&<span>貯玉追加消費: <b>+{_additionalStoredConsumed.toLocaleString()}玉</b>（{_storedSnapForRate.toLocaleString()}→{_storedAfterForRate.toLocaleString()}玉）<br/></span>}
